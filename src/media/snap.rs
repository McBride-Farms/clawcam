...
pub async fn run_snap(dev: &Device, out: Option<&str>) -> Result<()> {
    ...
    // Try rpicam-apps first (Pi 4/5 with Pi Camera Module — Pi OS default)
    ...
    if rpicam_result.is_ok() {
        // rpicam-still worked, download the image
    ...
    // Try GStreamer with libcamerasrc (Pi Camera Module via libcamera)
    ...
    // Try GStreamer with v4l2src (USB webcams, conference cams)
